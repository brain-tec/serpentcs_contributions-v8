...
class report_dynamic_label(report_sxw.rml_parse):
    ...
    def get_data(self,row,columns,ids,model,number_of_copy):
        ...
        for datas in active_model_obj.browse(self.cr, self.uid, ids, context=self.context):
            # get the data in the partner language if the active model is res.partner
            if model == 'res.partner':
                datas = active_model_obj.browse(self.cr, self.uid, datas.id, context={'lang': datas.lang or 'de_DE'})
            for i in range(0, number_of_copy):
                ...
                for field in label_print_data.field_ids:
                    current_newline = field.newline
                    # If after the last newline no data was print, deactivate the newline of this field
                    if last_newline:
                        current_newline = False
                        
                    if not field.python_field and not field.field_id:
                        if current_newline:
                            last_newline = True
                            vals.append({'newline': current_newline, 'only_newline': True, 'value': ''})
                        continue
                    pos=''
                    ...
                    if field.type == 'image' or field.type == 'barcode':
                        ...
                        if field.position != 'bottom':
                            ...
                            bot =True
                            bot_dict = {'string': string, 'value':  value, 'type': field.type, 'newline': current_newline, 'style': "font-size:"+str(field.fontsize)+"px;"+pos}
                    else:
                        bot = False
                    if not bot:
                        vals_dict = {'string': string, 'value':  value, 'type': field.type, 'newline': current_newline, 'style': "font-size:"+str(field.fontsize)+"px;"+pos}
...
#         result = result[0]
        ...
        for row in range(0, len(result)):
            new_val = result[row*columns: row*columns + columns]
            if new_val:
                new_list.append(new_val)
        return new_list
